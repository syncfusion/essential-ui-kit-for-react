...
export default function AIFloatingMenu2() {
    /* SB Code - Start */
    ...
    const handleMessageEvent = (event: MessageEvent) => {
        if (event.origin === window.location.origin) {
            try {
                ...
                if (blockData.name === 'ai-floating-menu-2' && blockData.theme) {
                            ...
                            <DialogComponent ref={dialogRef} id={styles["floating-contact"]} className="sm:rounded-lg rounded-none overflow-hidden !border-0" width="400px" height={isMobileView ? '580px' : '510px'} open={setDialogPosition} created={() => dialogRef.current?.show()}
                                ...
                                footerTemplate={() => (
                                    <div className="sm:px-6 px-4 py-6">
                                        <ButtonComponent className="w-full !ml-0" cssClass="e-primary" type="submit" content="Send"></ButtonComponent>
                                    </div>
                                )}
                            >
                                <div className="sm:px-6 px-4 pt-6 flex flex-col">
                                    ...
                                    <div className="w-full flex flex-col gap-1.5 mt-3">
                                        <UploaderComponent multiple={false} allowedExtensions="pdf/*" maxFileSize={2000000} buttons={{ browse: 'Upload' }} selected={(e) => { e.cancel = true; }}></UploaderComponent>
                                        <p className="text-xs text-gray-700 dark:text-gray-300">Supported formats: Doc/Pdf</p>
                                    </div>
                                </div>
                            </DialogComponent>
                            <FabComponent ref={fabRef} className="!hidden sm:!block" cssClass="e-primary e-round e-bigger" iconCss={!toggleState ? 'sf-icon-message-chat-circle !text-2xl' : 'e-icons e-close !text-2xl'} type="button" position="BottomRight" target="#target" onClick={toggleDialog}></FabComponent>
                        </div>
                    </section>
                );
            case 'bootstrap5':
