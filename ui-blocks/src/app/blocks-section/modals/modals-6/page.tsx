    ...
    const getContent = () => {
        switch (theme) {
            case 'tailwind':
                return (
                    <section>
                        <div id="dialog-container" className="relative" style={{ minHeight: "764px" }}>
                            <DialogComponent ref={dialog} key={"modal-6-tw"} id={styles["dialogs"]} className="rounded-none sm:rounded-lg sm:m-4 overflow-hidden" target="#dialog-container" beforeOpen={(event) => { event.maxHeight = '100%'; }} open={(e) => { e.preventFocus = true; }} showCloseIcon={true} width="835px" isModal={true}
                                header={() => (
                                ...
                                <div>
                                    <div className="flex flex-col sm:flex-row justify-between gap-4 sm:items-center mb-5 sm:mb-6">
                                        ...
                                        <div className="flex flex-row justify-end gap-3">
                                            <ButtonComponent cssClass="w-1/2 sm:w-fit e-outline" iconCss="e-icons e-export" content="Export" type="button"></ButtonComponent>
                                            <DropDownButtonComponent cssClass="w-1/2 sm:w-fit e-outline" iconCss="e-icons e-user" content="Contact" type="button" beforeOpen={(e) => e.cancel = true}></DropDownButtonComponent>
                                        </div>
                                    </div>
                                    <GridComponent cssClass="e-bigger mb-1.5" dataSource={data} width="100%" allowSorting={true} allowFiltering={true} filterSettings={{ type: 'Menu' }} gridLines="None" rowHeight={44}>
                                        <ColumnsDirective>
                                            ...
                                            <ColumnDirective field="customerDetails.name" headerText="Customer Name" textAlign="Left" width={260} template={(data: any) => (
                        ...
                        <div className="top-0 left-0 absolute w-full flex">
                            <ButtonComponent className="my-5 mx-auto" content="Transaction Details" type="button" onClick={() => dialog.current?.show()}></ButtonComponent>
                        </div>
                        {/* SB Code - End */}
                    </section>
                ); 
            case 'bootstrap5':
                return (
                    <section>
                        <div id="dialog-container" className="position-relative" style={{ minHeight: "764px" }}>
                            <DialogComponent ref={dialog} key={"modal-6-bs"} id={styles["dialogs"]} className="rounded-3 m-sm-2 overflow-hidden" target="#dialog-container" beforeOpen={(e) => { e.maxHeight = '100%' }} open={(e) => { e.preventFocus = true; }} showCloseIcon={true} width="865px" isModal={true}
                                header={() => (
                                ...
                                <div>
                                    <div className="d-flex flex-column flex-sm-row justify-content-between gap-3 align-items-sm-center mb-4 mb-sm-3">
                                        ...
                                        <div className="d-flex flex-row justify-content-end gap-3 gap-sm-2">
                                            <ButtonComponent className="col col-sm-auto e-outline" iconCss="e-icons e-export" content="Export" type="button"></ButtonComponent>
                                            <DropDownButtonComponent className="col col-sm-auto e-outline ms-sm-1" iconCss="e-icons e-user" content="Contact" type="button" beforeOpen={(e) => e.cancel = true}></DropDownButtonComponent>
                                        </div>
                                    </div>
                        ...
                        <div className="position-absolute top-0 start-0 d-flex w-100">
                            <ButtonComponent className="mx-auto my-3 e-outline" content="Transaction Details" type="button" onClick={() => dialog.current?.show()}></ButtonComponent>
                        </div>
                        {/* SB Code - End */}
                    </section>
                );
